...
<body>
		...
		<div class="table-responsive-md ">
			<div class="table-title">
				<div class="row">
					...
					<div class="col-sm-6 ">
						<a href="#" class="btn btn-info add-new float-right"
							data-toggle="modal" data-target="#WorkHourModal"
							data-action="AddWorkHour">
							Novo
						</a>
					</div>
				</div>
				...
				<tbody>
					<c:forEach var="wk" items="${listWk}">
						<tr>
							...
							<td id="tdStart-${wk.getId()}"><c:out
									value="${wk.getStart()}" /></td>
							<td id="tdEnd-${wk.getId()}"><c:out value="${wk.getEnd()}" /></td>
							<td><a class="edit" title="Edit" data-toggle="modal"
								data-target="#WorkHourModal" data-action="UpdateWorkHour"
								data-id="${wk.getId()}" href="#"><i class="material-icons">&#xE254;</i></a>
					...
					<form action="${linkEntryServlet}" method="post">
						<div class="form-group">
							<input id="modalDate" class="form-control" type="date"
								name="date" max="3000-12-31" min="1000-01-01">
						</div>
						<div class="form-group">
							<input id="modalStart" class="form-control" type="time"
								name="start" required>
						</div>
						<div class="form-group">
							<input id="modalEnd" class="form-control" type="time" name="end"
								required>
						</div>
	...
	<script type="text/javascript" src="assets/js/timesheet.js"></script>
</body>
</html>